...
running a prompt through the Forge runner (`forge3`) over its JSON-RPC stdio
protocol — see `scripts/forge_client.py`. Each call is a fresh single-turn
conversation whose assistant text is read back as the completion.
"""
...
from scripts.forge_client import run_prompt
from scripts.utils import parse_skill_md


def _complete(prompt: str, model: str | None, provider: str | None,
              timeout: int = 300) -> str:
    """Run one prompt through forge3 and return the assistant's text.

    The run happens in an empty temp directory so no project skills or repo
    context leak into what is meant to be a plain text completion.
    """
    with tempfile.TemporaryDirectory(prefix="forge-improve-") as cwd:
        result = run_prompt(
            prompt,
            cwd=cwd,
            timeout=timeout,
...
def improve_description(
    ...
    """Ask the agent to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _complete(prompt, model, provider)
    ...
    # SDK path did this as a true multi-turn; each forge3 run here starts a
    # fresh conversation, so we inline the prior output into the new prompt.)
    if len(description) > 1024:
        ...
        shorten_text = _complete(shorten_prompt, model, provider)
